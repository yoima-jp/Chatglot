...
public final class AnthropicModelCatalogService {
    private static final Logger LOGGER = LoggerFactory.getLogger(AnthropicModelCatalogService.class);
    private static final String MODELS_LIST_URL = "https://api.anthropic.com/v1/models";
    private static final String API_VERSION = "2023-06-01";
    private static final String CACHE_FILENAME = "anthropic_models.json";

    private final HttpClient httpClient = HttpClient.newHttpClient();
    private final Path cacheFile;
    private List<String> cachedModels = List.of();

    public AnthropicModelCatalogService(Path configDir) {
    ...
    public synchronized List<String> refreshModels(String apiKey, int timeoutSeconds) throws IOException, InterruptedException {
        if (apiKey == null || apiKey.isBlank()) {
            throw new IOException("Anthropic API key is empty");
        }
        ...
        if (models.isEmpty()) {
            throw new IOException("Anthropic model API returned no models");
        }
